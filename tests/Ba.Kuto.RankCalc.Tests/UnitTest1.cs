...
public class RankCalculatorTests
{
    ...
    // --- Mainメソッドのテスト (戦略引数に対応) ---
    [Theory]
    [InlineData("100", "optimal", 100, 9, new int[] { 100, 70, 49, 34, 23, 16, 11, 7, 4, 1 })]
    [InlineData("100", "compromise", 100, 9, new int[] { 100, 78, 54, 37, 25, 17, 11, 7, 4, 1 })]
    [InlineData("13", "optimal", 13, 4, new int[] { 13, 9, 6, 3, 1 })]
    [InlineData("13", "compromise", 13, 4, new int[] { 13, 11, 7, 4, 1 })]
    public void Main_WithStrategy_ShouldOutputCorrectJson(string rankArg, string strategyArg, int expectedStartRank, int expectedBattleCount, int[] expectedRoute)
    {
        using var sw = new StringWriter();
        Console.SetOut(sw);

        RankCalculator.Main([rankArg, strategyArg]);

        var output = sw.ToString().Trim();

        try
        {
            var result = JsonSerializer.Deserialize<CalculationResult>(output);
            Assert.NotNull(result);
            Assert.Equal(expectedStartRank, result.StartRank);
            Assert.Equal(expectedBattleCount, result.BattleCount);
            Assert.Equal(expectedRoute, result.Route);
        }
        catch (JsonException ex)
        {
            Assert.Fail($"出力が有効なJSONではありません: {ex.Message}\nOutput: {output}");
        }
    }

    [Fact]
    public void Main_NoArgs_ShouldOutputError()
    {
        using var sw = new StringWriter();
        Console.SetOut(sw);
        RankCalculator.Main([]);
        var output = sw.ToString().Trim();
        Assert.Contains("エラー: 計算対象の順位を引数として指定してください。", output);
    }

    [Fact]
    public void Main_InvalidRank_ShouldOutputError()
    {
        using var sw = new StringWriter();
        Console.SetOut(sw);
        RankCalculator.Main(["abc"]);
        var output = sw.ToString().Trim();
        Assert.Contains("エラー: 引数には1以上の整数を指定してください。", output);
    }

    [Fact]
    public void Main_SuggestStrategy_ShouldOutputBothRoutesWhenDifferent()
    {
        using var sw = new StringWriter();
        Console.SetOut(sw);

        RankCalculator.Main(["13"]); // 13位はoptimalとcompromiseが異なる

        var output = sw.ToString().Trim();

        try
        {
            var result = JsonSerializer.Deserialize<SuggestedRoutesResult>(output);
            Assert.NotNull(result);
            Assert.NotNull(result.OptimalRoute);
            Assert.NotNull(result.CompromiseRoute);
            Assert.Equal(13, result.OptimalRoute.StartRank);
            Assert.Equal(4, result.OptimalRoute.BattleCount);
            Assert.Equal([13, 9, 6, 3, 1], result.OptimalRoute.Route);
            Assert.Equal(13, result.CompromiseRoute.StartRank);
            Assert.Equal(4, result.CompromiseRoute.BattleCount);
            Assert.Equal([13, 11, 7, 4, 1], result.CompromiseRoute.Route);
        }
        catch (JsonException ex)
        {
            Assert.Fail($"出力が有効なJSONではありません: {ex.Message}\nOutput: {output}");
    ...
    [Fact]
    public void Main_SuggestStrategy_ShouldOutputBothRoutesWhenDifferentFor100()
    {
        using var sw = new StringWriter();
        Console.SetOut(sw);

        RankCalculator.Main(["100"]); // 100位はoptimalとcompromiseが異なる
        ...
        try
        {
            ...
            Assert.NotNull(result);
            Assert.NotNull(result.OptimalRoute);
            Assert.NotNull(result.CompromiseRoute); // 異なるのでnullではない
            Assert.Equal(100, result.OptimalRoute.StartRank);
            Assert.Equal(9, result.OptimalRoute.BattleCount);
            Assert.Equal([100, 70, 49, 34, 23, 16, 11, 7, 4, 1], result.OptimalRoute.Route);
            Assert.Equal(100, result.CompromiseRoute.StartRank);
            Assert.Equal(9, result.CompromiseRoute.BattleCount);
            Assert.Equal([100, 78, 54, 37, 25, 17, 11, 7, 4, 1], result.CompromiseRoute.Route);
        }
        catch (JsonException ex)
        {
            Assert.Fail($"出力が有効なJSONではありません: {ex.Message}\nOutput: {output}");
