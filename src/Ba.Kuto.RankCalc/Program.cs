...
public record CalculationResult(int StartRank, int BattleCount, List<int> Route);

public record SuggestedRoutesResult(CalculationResult OptimalRoute, CalculationResult? CompromiseRoute);
...
public static class RankCalculator
{
    public static int Main(string[] args)
    {
        ...
        if (args.Length < 1)
        {
            Console.WriteLine("エラー: 計算対象の順位を引数として指定してください。");
            Console.WriteLine("使用法: dotnet run -- <現在の順位> [optimal|compromise|max_battles]");
            goto FAILED;
        }

        if (!int.TryParse(args[0], out int startRank) || startRank < 1)
        {
            Console.WriteLine("エラー: 引数には1以上の整数を指定してください。");
            goto FAILED;
        }

        string strategy = "suggest"; // デフォルトは提案モード
        if (args.Length > 1)
        {
            strategy = args[1].ToLowerInvariant();
        }

        string jsonOutput;
        if (strategy == "suggest")
        {
            var optimalRoute = CalculateOptimalRoute(startRank);
            var compromiseRoute = CalculateCompromiseRoute(startRank, optimalRoute);

            var suggestedResult = new SuggestedRoutesResult(
                new(startRank, optimalRoute.Count - 1, optimalRoute),
                optimalRoute.SequenceEqual(compromiseRoute) // ルートが異なる場合のみ妥協ルートを追加
                    ? default
                    : new(startRank, compromiseRoute.Count - 1, compromiseRoute));

            jsonOutput = JsonSerializer.Serialize(suggestedResult);
        }
        else
        {
            var route = strategy switch
            {
                "compromise" => CalculateCompromiseRoute(startRank),
                "max_battles" => CalculateMaxBattleRoute(startRank),
                _ => CalculateOptimalRoute(startRank),
            };
            var result = new CalculationResult(startRank, route.Count - 1, route);

            jsonOutput = JsonSerializer.Serialize(result);
        }
